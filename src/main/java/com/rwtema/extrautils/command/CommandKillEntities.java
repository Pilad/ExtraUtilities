...
public class CommandKillEntities extends CommandBase {
    ...
    public void processCommand(final ICommandSender icommandsender, final String[] astring) {
        ...
        for (int j = 0; j < MinecraftServer.getServer().worldServers.length; ++j) {
            this.killEntities(MinecraftServer.getServer().worldServers[j]);
        }
        icommandsender.addChatMessage(new ChatComponentText("Killed " + this.numKills + " of type " + this.entityclass.getName()));
    }
