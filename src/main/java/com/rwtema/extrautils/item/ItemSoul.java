...
public class ItemSoul extends Item {
    ...
    public ItemStack onItemRightClick(final ItemStack item, final World par3World, final EntityPlayer player) {
        ...
        player.addChatComponentMessage(new ChatComponentText("You feel strangely refreshed (+10% Max Health)"));
        --item.stackSize;
        return item;
    }
    ...
    public void onCreated(final ItemStack par1ItemStack, final World par2World, final EntityPlayer player) {
        ...
        if (c >= 6.0) {
            ...
            if (!par2World.isRemote) {
                ...
                multimap.put(SharedMonsterAttributes.maxHealth.getAttributeUnlocalizedName(), new AttributeModifier(ItemSoul.uuid, "Missing Soul", l, 2));
                player.getAttributeMap().applyAttributeModifiers(multimap);
                player.addChatComponentMessage(new ChatComponentText("You feel diminished (-10% Max Health)"));
            }
        }
