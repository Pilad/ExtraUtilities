...
public class ItemWateringCan extends Item {
    ...
    private void waterLocation(final World worldObj, final double hitX, final double hitY, final double hitZ, final int side, final ItemStack stack, final EntityPlayer play, final int range) {
        final List enderman = worldObj.getEntitiesWithinAABB(EntityEnderman.class, AxisAlignedBB.getBoundingBox(hitX - range, hitY - range, hitZ - range, hitX + range, hitY + 6.0, hitZ + range));
        if (enderman != null) {
        ...
        if (worldObj.isRemote) {
            ...
            final List ents = worldObj.getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getBoundingBox(hitX - range, hitY - range, hitZ - range, hitX + range, hitY + range + 6.0, hitZ + range));
            if (ents != null) {
