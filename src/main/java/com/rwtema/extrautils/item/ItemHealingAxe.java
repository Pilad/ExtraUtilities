        ...
        if (entity instanceof EntityLiving) {
            if (entity.worldObj.isRemote) {
                for (int i = 0; i < 5; ++i) {
                    ...
                    entity.worldObj.spawnParticle("mobSpell", entity.posX + (entity.worldObj.rand.nextDouble() - 0.5) * entity.width, entity.posY + entity.worldObj.rand.nextDouble() * entity.height - entity.yOffset, entity.posZ + (entity.worldObj.rand.nextDouble() - 0.5) * entity.width, r, g, b);
                }
            }
            if (k > 0.0f) {
                ...
                if (((EntityLiving) entity).isEntityUndead() && entity.isEntityAlive()) {
                    if (entity instanceof EntityZombie && ((EntityZombie) entity).isVillager()) {
                        if (!entity.worldObj.isRemote) {
                            entity.getDataWatcher().updateObject(14, (byte) 1);
