...
public class PartFence extends PartConnecting implements IFence {
    ...
    public boolean shouldConnect(final int x, final int y, final int z, final int direction) {
        ...
        if (this.world().getTileEntity(x, y, z) instanceof IFence && this.tile().canAddPart(PartFenceDummyArm.dummyArms[direction])) {
            return ((TileMultipart) this.world().getTileEntity(x, y, z)).canAddPart(PartFenceDummyArm.dummyArms[Facing.oppositeSide[direction]]);
        }
        return l.getMaterial().isOpaque() && l.renderAsNormalBlock() && this.tile().canAddPart(PartFenceDummyArm.dummyArms[direction]);
    }
    ...
    public boolean renderStatic(final Vector3 pos, final int pass) {
        ...
        if (this.mat != null && this.mat.canRenderInPass(pass)) {
            ...
            for (int i = 0; i < 6; ++i) {
                if ((this.connectionMask & 1 << i) != 0x0) {
                    MicroblockRender.renderCuboid(new Vector3(this.x(), this.y(), this.z()), this.mat, pass, PartFence.renderCuboids1[i], 1 << Facing.oppositeSide[i] | 1 << i);
                    MicroblockRender.renderCuboid(new Vector3(this.x(), this.y(), this.z()), this.mat, pass, PartFence.renderCuboids2[i], 1 << Facing.oppositeSide[i] | 1 << i);
                }
            }
            return true;
        }
