...
public class RegisterBlockPart implements MultiPartRegistry.IPartConverter, MultiPartRegistry.IPartFactory {
    ...
    public RegisterBlockPart(final Block block, final Class<? extends TMultiPart> part) {
        ...
        this.name = "";
        try {
            this.name = part.getConstructor((Class<?>[]) new Class[0]).newInstance(new Object[0]).getType();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public TMultiPart createPart(final String name, final boolean client) {
        if (name.equals(name)) {
            try {
                return this.part.getConstructor((Class<?>[]) new Class[0]).newInstance(new Object[0]);
            } catch (Exception e) {
                e.printStackTrace();
                return null;
            }
