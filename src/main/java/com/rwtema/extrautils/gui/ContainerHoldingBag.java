...
public class ContainerHoldingBag extends Container {
    ...
    public ContainerHoldingBag(final EntityPlayer player, final int invId) {
        ...
        for (int j = 0; j < 6; ++j) {
            for (int k = 0; k < 9; ++k) {
                this.addSlotToContainer(new SlotItemContainer(player.inventory, k + j * 9, 8 + k * 18, 18 + j * 18, this.currentFilter));
            }
        }
        for (int j = 0; j < 3; ++j) {
            for (int k = 0; k < 9; ++k) {
                this.addSlotToContainer(new Slot(player.inventory, k + j * 9 + 9, 8 + k * 18, 104 + j * 18 + i));
            }
        }
        for (int j = 0; j < 9; ++j) {
            if (j == this.currentFilter) {
                this.addSlotToContainer(new SlotDisabled(player.inventory, j, 8 + j * 18, 162 + i));
            } else {
                this.addSlotToContainer(new Slot(player.inventory, j, 8 + j * 18, 162 + i));
            }
        }
    }
