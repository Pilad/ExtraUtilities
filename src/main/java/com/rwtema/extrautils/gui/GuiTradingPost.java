...
public class GuiTradingPost extends GuiScreen {
    ...
    protected void drawItemStackTooltip(final ItemStack par1ItemStack, final int par2, final int par3) {
        ...
        GL11.glDisable(2896);
        GL11.glDisable(2929);
        final List<String> list = par1ItemStack.getTooltip(this.mc.thePlayer, this.mc.gameSettings.advancedItemTooltips);
        if (!list.isEmpty()) {
            int k = 0;
            for (int l = 0; l < list.size(); ++l) {
                final int i1 = this.fontRendererObj.getStringWidth(list.get(l));
                if (i1 > k) {
                    k = i1;
                }
            }
