...
import com.rwtema.extrautils.block.*;
import com.rwtema.extrautils.command.CommandKillEntities;
import com.rwtema.extrautils.crafting.*;
...
import com.rwtema.extrautils.tileentity.*;
...
import com.rwtema.extrautils.tileentity.enderquarry.*;
...
public class ExtraUtils {
    ...
    public static void addDungeonItem(final ItemStack item, final int min, final int max, final String category, final double probability) {
        final int n = getWeightTotal(ChestGenHooks.getItems(category, XURandom.getInstance()));
    ...
    public static void addRecipe(final ItemStack out, final Object... ingreds) {
        GameRegistry.addRecipe(new ShapedOreRecipe(out, ingreds));
    }
            ...
            for (final ModCandidate t : event.getAsmData().getCandidatesFor("cofh.api.energy")) {
                ...
                if (t.getClassList().contains("cofh/api/energy/IEnergyHandler") && !t.getClassList().contains("cofh/api/energy/IEnergyConnection")) {
                    for (final ModContainer mod : t.getContainedMods()) {
                        LogHelper.info("" + mod.getModId() + " (" + mod.getName() + ") appears to be missing the updated COFH api");
                    }
                }
            }
            ...
            for (final ModCandidate t2 : event.getAsmData().getCandidatesFor("cofh.api.energy")) {
                ...
                if (t2.getClassList().contains("cofh/api/energy/IEnergyHandler") && !t2.getClassList().contains("cofh/api/energy/IEnergyConnection")) {
                    for (final ModContainer mod2 : t2.getContainedMods()) {
                        LogHelper.info("" + mod2.getModId() + " (" + mod2.getName() + ") appears to be missing the updated COFH api");
                    }
            ...
            for (String s : t3.getClassList()) {
                ...
                if (s.startsWith(networkPath)) {
                    try {
                        ...
                        LogHelper.error(s + " can't be created");
    ...
    public void setupConfigs(final File file) {
            ...
            for (int i = prev_version; i < toAdd.length; ++i) {
                if (toAdd[i] != null) {
                    for (final String s : toAdd[i]) {
                        if (strings.add(s)) {
                            LogHelper.info("QEDCrafting Updater: Added Recipe - " + s);
                        }
                    }
                }
                if (toRemove[i] != null) {
                    for (final String s : toRemove[i]) {
                        if (strings.remove(s)) {
                            LogHelper.info("QEDCrafting Updater: Removed Recipe - " + s);
                        }
                    }
                }
            }
        ...
        prop.set(strings.toArray(new String[strings.size()]));
        if (strings.size() > 0) {
            for (final String s2 : strings) {
                ExtraUtils.qedList.add(s2);
                LogHelper.fine("Recipes constructing " + s2 + " will now be used in the Ender Constructor");
            }
        } else {
            ExtraUtils.qedEnabled = false;
            LogHelper.fine("No Recipes for QED found. QED will be disabled.");
        }
    ...
    public void registerStuff() {
        ...
        if (ExtraUtils.gloveEnabled) {
            registerItem(ExtraUtils.glove = new ItemGlove());
        }
        ...
        if (ExtraUtils.decorative1Enabled && ExtraUtils.decorative2Enabled) {
            ...
            ExtraUtils.decorative1.hardness[1] = Blocks.obsidian.getBlockHardness(null, 0, 0, 0);
            ExtraUtils.decorative1.resistance[1] = Blocks.obsidian.getExplosionResistance(null) * 5.0f;
            ...
            OreDictionary.registerOre("bricksGravel", new ItemStack(ExtraUtils.decorative1, 1, 6));
            OreDictionary.registerOre("blockEnderObsidian", new ItemStack(ExtraUtils.decorative1, 1, 1));
            OreDictionary.registerOre("burntQuartz", new ItemStack(ExtraUtils.decorative1, 1, 2));
            OreDictionary.registerOre("blockIcestone", new ItemStack(ExtraUtils.decorative1, 1, 3));
            OreDictionary.registerOre("blockUnstable", new ItemStack(ExtraUtils.decorative1, 1, 5));
            OreDictionary.registerOre("blockMagicWood", new ItemStack(ExtraUtils.decorative1, 1, 7));
            OreDictionary.registerOre("blockEnderCore", new ItemStack(ExtraUtils.decorative1, 1, 11));
            OreDictionary.registerOre("blockGlassSandy", new ItemStack(ExtraUtils.decorative1, 1, 9));
            ...
            for (int i = 0; i < 12; ++i) {
                if (i != 4 && i != 5 && i != 7 && i != 8 && i != 10 && i != 11) {
                    OreDictionary.registerOre("blockGlass", new ItemStack(ExtraUtils.decorative2, 1, i));
                }
            }
        }
        ...
        if (ExtraUtils.bedrockiumBlockEnabled) {
            registerBlock(ExtraUtils.bedrockiumBlock = new BlockBedrockium(), BlockBedrockium.ItemBedrockium.class);
        }
        ...
        if (ExtraUtils.cobblestoneComprEnabled) {
            ...
            for (int i = 0; i < 16; ++i) {
                ...
                OreDictionary.registerOre(s, new ItemStack(ExtraUtils.cobblestoneCompr, 1, i));
            }
        }
        ...
        if (ExtraUtils.tradingPostEnabled) {
            registerBlock(ExtraUtils.tradingPost = new BlockTradingPost());
        ...
        if (ExtraUtils.enderThermicPumpEnabled) {
            ...
            registerTile(TileEntityEnderThermicLavaPump.class, "enderPump");
        }
        if (ExtraUtils.enderQuarryEnabled) {
        ...
        if (ExtraUtils.enderLilyEnabled) {
            registerBlock(ExtraUtils.enderLily = new BlockEnderLily(), ItemBlockEnderLily.class);
        ...
        if (ExtraUtils.magnumTorchEnabled) {
            ...
            if (Loader.isModLoaded("ForgeMultipart")) {
                new RegisterBlockPart(ExtraUtils.magnumTorch, MagnumTorchPart.class, "XU|MagnumTorch").init();
            }
        }
        ...
        if (ExtraUtils.lawSwordEnabled) {
            registerItem(ExtraUtils.lawSword = new ItemLawSword());
        }
        ...
        if (ExtraUtils.portalEnabled && (ExtraUtils.endoftimeDimID != 0 || ExtraUtils.underdarkDimID != 0)) {
            ...
            if (ExtraUtils.endoftimeDimID != 0) {
                if (DimensionManager.isDimensionRegistered(ExtraUtils.endoftimeDimID)) {
                    ExtraUtilsMod.proxy.throwLoadingError("Invalid id for 'End of Time' dimension. Change endoftimeDimID in config.");
                }
                DimensionManager.registerProviderType(ExtraUtils.endoftimeDimID, WorldProviderEndOfTime.class, true);
                DimensionManager.registerDimension(ExtraUtils.endoftimeDimID, ExtraUtils.endoftimeDimID);
            }
            if (ExtraUtils.underdarkDimID != 0) {
                DimensionManager.registerProviderType(ExtraUtils.underdarkDimID, WorldProviderUnderdark.class, false);
        ...
        if (ExtraUtils.precisionShearsEnabled) {
            registerItem(ExtraUtils.precisionShears = new ItemPrecisionShears());
        }
        ...
        if (ExtraUtils.ethericSwordEnabled) {
            registerItem(ExtraUtils.ethericSword = new ItemEthericSword());
        }
        if (ExtraUtils.erosionShovelEnabled) {
            registerItem(ExtraUtils.erosionShovel = new ItemErosionShovel());
        }
        if (ExtraUtils.destructionPickaxeEnabled) {
            registerItem(ExtraUtils.destructionPickaxe = new ItemDestructionPickaxe());
        }
        if (ExtraUtils.healingAxeEnabled) {
            registerItem(ExtraUtils.healingAxe = new ItemHealingAxe());
        }
        if (ExtraUtils.temporalHoeEnabled) {
            registerItem(ExtraUtils.temporalHoe = new ItemTemporalHoe());
        }
        if (ExtraUtils.sonarGogglesEnabled) {
            registerItem(ExtraUtils.sonarGoggles = new ItemSonarGoggles());
        }
    ...
    public void init(final FMLInitializationEvent event) {
        NetworkRegistry.INSTANCE.registerGuiHandler(ExtraUtilsMod.instance, new GuiHandler());
        if (Loader.isModLoaded("ForgeMultipart")) {
    ...
    public void postInit(final FMLPostInitializationEvent evt) {
        ForgeChunkManager.setForcedChunkLoadingCallback(ExtraUtilsMod.instance, new ChunkloadCallback());
        ...
        if (!ExtraUtils.disableEnderLiliesInDungeons && ExtraUtils.enderLily != null) {
            addDungeonItem(new ItemStack(ExtraUtils.enderLily), 1, 5, "dungeonChest", 0.03);
        }
    ...
    public void serverStarting(final FMLServerStartingEvent event) {
        event.registerServerCommand(new CommandKillEntities("items", EntityItem.class, true));
        event.registerServerCommand(new CommandKillEntities("mobs", EntityMob.class, true));
        event.registerServerCommand(new CommandKillEntities("living", EntityLiving.class, true));
        event.registerServerCommand(new CommandKillEntities("xp", EntityXPOrb.class, true));
    }

    public void registerRecipes() {
        this.registerRecipe(ShapedOreRecipeAlwaysLast.class, RecipeSorter.Category.SHAPED, "after:forge:shapelessore");
        this.registerRecipe(ShapelessOreRecipeAlwaysLast.class, RecipeSorter.Category.SHAPELESS, "after:forge:shapelessore");
        final String unstableIngot = "ingotUnstable";
        if (ExtraUtils.fullChestEnabled) {
            addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.fullChest), new Object[]{"sss", "sCs", "sss", 's', "stickWood", 'C', Blocks.chest}));
        }
        if (ExtraUtils.miniChestEnabled) {
            addRecipe(new ShapelessOreRecipeAlwaysLast(new ItemStack(ExtraUtils.miniChest, 9), new Object[]{Blocks.chest}));
            if (ExtraUtils.fullChestEnabled) {
                addRecipe(new ShapelessOreRecipe(new ItemStack(ExtraUtils.miniChest, 9), new Object[]{ExtraUtils.fullChest}));
            }
            this.addShapedRecipe(new ItemStack(Blocks.chest), "ccc", "ccc", "ccc", 'c', ExtraUtils.miniChest);
        }
        if (ExtraUtils.enderCollectorEnabled) {
        ...
        if (ExtraUtils.gloveEnabled) {
            addRecipe(new RecipeGlove(ExtraUtils.glove));
        }
        if (ExtraUtils.heatingCoilEnabled) {
            addRecipe(new ItemStack(ExtraUtils.heatingCoil), "III", "I I", "IRI", 'I', Blocks.heavy_weighted_pressure_plate, 'R', Items.redstone);
        }
        if (ExtraUtils.microBlocksEnabled) {
            addRecipe(new RecipeMicroBlocks(3, 3, new Object[]{1, 1, 1, 1, new ItemStack(Blocks.wool, 1), 1, 1, 1, 1}, new ItemStack(ExtraUtils.microBlocks, 8)));
            addRecipe(new RecipeMicroBlocks(3, 3, new Object[]{770, 772, 770, 770, 772, 770, null, 772, null}, new ItemStack(ExtraUtils.microBlocks, 2, 1)));
            addRecipe(new RecipeMicroBlocks(3, 3, new Object[]{null, 0, null, 4, 0, 4, 4, 0, 4}, new ItemStack(ExtraUtils.microBlocks, 5, 2)));
            addRecipe(new RecipeMicroBlocks(3, 3, new Object[]{1, 4, 1, 4, new ItemStack(ExtraUtils.decorative1, 1, 5), 4, 1, 4, 1}, new ItemStack(ExtraUtils.microBlocks, 1, 3)));
        }
        if (ExtraUtils.bedrockiumBlockEnabled && ExtraUtils.bedrockiumEnabled) {
            addRecipe(new ItemStack(ExtraUtils.bedrockiumBlock), "KKK", "KKK", "KKK", 'K', ExtraUtils.bedrockium);
            addRecipe(new ItemStack(ExtraUtils.bedrockium, 9), "K", 'K', ExtraUtils.bedrockiumBlock);
            if (ExtraUtils.cobblestoneComprEnabled) {
                this.addSmelting(new ItemStack(ExtraUtils.cobblestoneCompr, 1, 7), new ItemStack(ExtraUtils.bedrockiumBlock), 0);
            }
        }
        if (ExtraUtils.soulEnabled && ExtraUtils.ethericSwordEnabled) {
            addRecipe(new RecipeSoul());
        }
        ...
        if (ExtraUtils.BUDBlockEnabled) {
            if (!ExtraUtils.disableBUDBlockRecipe) {
                addRecipe(new ItemStack(ExtraUtils.BUDBlock, 1, 0), "SRS", "SPS", "STS", 'R', Items.redstone, 'S', Blocks.stone, 'P', Blocks.sticky_piston, 'T', Blocks.redstone_torch);
            }
            if (!ExtraUtils.disableAdvBUDBlockRecipe) {
                addRecipe(new ItemStack(ExtraUtils.BUDBlock, 1, 3), "SRS", "RBR", "SRS", 'R', Blocks.redstone_block, 'S', Blocks.stone, 'B', new ItemStack(ExtraUtils.BUDBlock, 1, 0));
            }
        }
        ...
        if (ExtraUtils.decorative1Enabled && ExtraUtils.decorative2Enabled) {
            addRecipe(new ItemStack(ExtraUtils.decorative1, 9, 0), "SBS", "BBB", "SBS", 'S', Blocks.stone, 'B', new ItemStack(Blocks.stonebrick, 1, 0));
            addRecipe(new ItemStack(ExtraUtils.decorative1, 4, 1), " O ", "OEO", " O ", 'O', Blocks.obsidian, 'E', Items.ender_pearl);
            this.addSmelting(new ItemStack(Blocks.quartz_block), new ItemStack(ExtraUtils.decorative1, 1, 2), 0);
            addRecipe(new ItemStack(ExtraUtils.decorative1, 5, 3), " I ", "ISI", " I ", 'I', Blocks.ice, 'S', Blocks.stone);
            addRecipe(new ItemStack(ExtraUtils.decorative1, 4, 4), "BB", "BB", 'B', new ItemStack(ExtraUtils.decorative1, 1, 0));
            addRecipe(new ItemStack(ExtraUtils.decorative1, 4, 7), "BB", "BB", 'B', new ItemStack(ExtraUtils.decorative1, 1, 4));
            addRecipe(new ItemStack(ExtraUtils.decorative1, 1, 6), "GG", "GG", 'G', Blocks.gravel);
            addRecipe(new RecipeMagicalWood());
            addRecipe(new ItemStack(ExtraUtils.decorative1, 8, 10), "SGS", "GGG", "SGS", 'S', new ItemStack(Blocks.stone_slab, 1, 5), 'G', new ItemStack(ExtraUtils.decorative1, 1, 6));
            addRecipe(new ItemStack(ExtraUtils.decorative1, 1, 11), "GBG", "BDB", "GBG", 'G', new ItemStack(ExtraUtils.decorative1, 1, 8), 'D', Items.ender_eye, 'B', new ItemStack(ExtraUtils.decorative1, 1, 1));
            addRecipe(new ItemStack(ExtraUtils.decorative1, 1, 12), "dEd", "EDE", "dEd", 'D', new ItemStack(ExtraUtils.decorative1, 1, 1), 'E', Items.diamond, 'd', new ItemStack(ExtraUtils.decorative1, 1, 2));
            addRecipe(new ShapelessOreRecipe(new ItemStack(ExtraUtils.decorative1, 4, 13), new Object[]{Blocks.end_stone, "sandstone", "sandstone", Blocks.end_stone}));
            addRecipe(new ShapelessOreRecipe(new ItemStack(ExtraUtils.decorative1, 4, 14), new Object[]{"dyeMagenta", "dyePurple", Blocks.stone, Blocks.stone, Blocks.stone, Blocks.stone, Items.ender_pearl}));
            this.addShapedRecipe(new ItemStack(ExtraUtils.decorative1, 4, 9), "SG", "GS", 'G', Blocks.glass, 'S', Blocks.sand);
            this.addSmelting(new ItemStack(ExtraUtils.decorative1, 1, 9), new ItemStack(ExtraUtils.decorative2, 1, 0), 0);
            final ItemStack glass = new ItemStack(ExtraUtils.decorative2, 1, 0);
            addRecipe(new ItemStack(ExtraUtils.decorative2, 8, 1), "GGG", "G G", "GGG", 'G', glass);
            addRecipe(new ItemStack(ExtraUtils.decorative2, 4, 2), "GG", "GG", 'G', glass);
            addRecipe(new ItemStack(ExtraUtils.decorative2, 1, 3), "GP", 'G', glass, 'P', Items.gunpowder);
            addRecipe(new ItemStack(ExtraUtils.decorative2, 1, 4), "ggg", "gGg", "ggg", 'G', glass, 'g', Items.gold_nugget);
            addRecipe(new ItemStack(ExtraUtils.decorative2, 4, 5), "GOG", "O O", "GOG", 'G', glass, 'O', Blocks.obsidian);
            addRecipe(new ItemStack(ExtraUtils.decorative2, 5, 6), " G ", "GGG", " G ", 'G', glass);
            addRecipe(new ItemStack(ExtraUtils.decorative2, 1, 7), " g ", "gGg", " g ", 'G', glass, 'g', Items.glowstone_dust);
            addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.decorative2, 6, 8), new Object[]{"GpG", "GGG", " G ", 'G', glass, 'p', "dyePink"}));
            addRecipe(new ItemStack(ExtraUtils.decorative2, 1, 9), "G", 'G', glass);
            addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.decorative2, 5, 10), new Object[]{"GCG", "CGC", "GCG", 'G', glass, 'C', (ExtraUtils.curtain == null) ? Items.coal : ExtraUtils.curtain, 'I', "dyeBlack"}));
            addRecipe(new ItemStack(ExtraUtils.decorative2, 4, 11), "GOG", "O O", "GOG", 'G', new ItemStack(ExtraUtils.decorative2, 1, 10), 'O', Blocks.obsidian);
        }
        ...
        if (ExtraUtils.colorBlockDataEnabled && ExtraUtils.paintBrushEnabled) {
            ...
            for (final BlockColor b : ExtraUtils.colorblocks) {
                for (int i = 0; i < 16; ++i) {
                    if (b.oreName != null) {
                        OreDictionary.registerOre(b.oreName, new ItemStack(b, 1, i));
                        OreDictionary.registerOre(b.oreName + XUHelper.dyes[i].substring(3), new ItemStack(b, 1, i));
                    }
                    if (!ExtraUtils.disableColoredBlocksRecipes) {
                        if (b.customRecipe == null) {
                            addRecipe(new ShapedOreRecipe(new ItemStack(b, 7, BlockColored.func_150032_b(i)), new Object[]{"SSS", "SDS", "SPS", 'S', b.baseBlock, 'D', XUHelper.dyes[i], 'P', ExtraUtils.paintBrush}));
                        } else {
                            ...
                            addRecipe(new ShapedOreRecipe(new ItemStack(b, b.customRecipeNo, BlockColored.func_150032_b(i)), tempRecipe));
                        }
                    }
                }
            }
        ...
        if (ExtraUtils.cobblestoneComprEnabled && !ExtraUtils.disableCompressedCobblestoneRecipe) {
            for (int k = 0; k < 16; ++k) {
                if (BlockCobblestoneCompressed.getCompr(k) == 0) {
                    ...
                    addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.cobblestoneCompr, 1, k), new Object[]{"XXX", "XXX", "XXX", 'X', s}));
                    addRecipe(new ItemStack(BlockCobblestoneCompressed.getBlock(k), 9), "X", 'X', new ItemStack(ExtraUtils.cobblestoneCompr, 1, k));
                } else {
                    addRecipe(new ItemStack(ExtraUtils.cobblestoneCompr, 1, k), "XXX", "XXX", "XXX", 'X', new ItemStack(ExtraUtils.cobblestoneCompr, 1, k - 1));
                    addRecipe(new ItemStack(ExtraUtils.cobblestoneCompr, 9, k - 1), "X", 'X', new ItemStack(ExtraUtils.cobblestoneCompr, 1, k));
                }
            }
        }
        ...
        if (ExtraUtils.filingCabinetEnabled) {
            if (!ExtraUtils.disableFilingCabinet) {
                addRecipe(new ItemStack(ExtraUtils.filingCabinet, 1, 0), "ICI", "ICI", "ICI", 'I', Items.iron_ingot, 'C', Blocks.chest);
            }
            if (!ExtraUtils.disableAdvFilingCabinet && ExtraUtils.decorative1Enabled) {
                addRecipe(new ItemStack(ExtraUtils.filingCabinet, 1, 1), "ICI", "ICI", "ICI", 'I', new ItemStack(ExtraUtils.decorative1, 1, 8), 'C', new ItemStack(ExtraUtils.filingCabinet, 1, 0));
            }
        }
        ...
        if (ExtraUtils.transferNodeEnabled && ExtraUtils.transferPipeEnabled && ExtraUtils.transferNodeRemoteEnabled) {
            if (!ExtraUtils.disableTransferPipeRecipe) {
                addRecipe(new ItemStack(ExtraUtils.transferPipe, 8), "SSS", "GRG", "SSS", 'S', new ItemStack(Blocks.stone_slab, 6, 0), 'G', Blocks.glass, 'R', Items.redstone);
            }
            if (!ExtraUtils.disableSortingPipeRecipe) {
                addRecipe(new ItemStack(ExtraUtils.transferPipe, 2, 8), "SSS", "G#G", "SSS", 'S', new ItemStack(Blocks.stone_slab, 6, 0), 'G', Blocks.glass, '#', Items.gold_ingot);
            }
            if (!ExtraUtils.disableFilterPipeRecipe) {
                ...
                addRecipe(new RecipeCustomOres(new ItemStack(ExtraUtils.transferPipe, 5, 9), new ItemStack(Items.stick), dyes, new Object[]{"sPs", "PPP", "sPs", 's', new ItemStack(Items.stick), 'P', new ItemStack(ExtraUtils.transferPipe, 1, 0)}));
            }
            if (!ExtraUtils.disableRationingPipeRecipe) {
                addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.transferPipe, 2, 10), new Object[]{"SSS", "GBG", "SSS", 'B', new ItemStack(Items.dye, 1, 4), 'S', new ItemStack(Blocks.stone_slab, 1, 0), 'G', Blocks.glass}));
                addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.transferPipe2, 2, 0), new Object[]{"SSS", "GBG", "SSS", 'B', new ItemStack(Items.dye, 1, 4), 'S', new ItemStack(Blocks.stone_button, 1, 0), 'G', Blocks.glass}));
            }
            if (!ExtraUtils.disableEnergyPipeRecipe) {
                addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.transferPipe, 8, 11), new Object[]{"SSS", "RRR", "SSS", 'R', Items.redstone, 'S', new ItemStack(Blocks.stone_slab, 6, 0)}));
            }
            if (!ExtraUtils.disableCrossoverPipeRecipe) {
                addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.transferPipe, 1, 12), new Object[]{" P ", "PBP", " P ", 'P', new ItemStack(ExtraUtils.transferPipe, 1, 0), 'B', Items.redstone}));
            }
            if (!ExtraUtils.disableModSortingPipesRecipe) {
                addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.transferPipe, 1, 13), new Object[]{"BPB", 'P', new ItemStack(ExtraUtils.transferPipe, 1, 8), 'B', Items.redstone}));
            }
            if (!ExtraUtils.disableEnergyExtractionPipeRecipe) {
                addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.transferPipe, 1, 14), new Object[]{"B B", "BPB", " P ", 'P', new ItemStack(ExtraUtils.transferPipe, 1, 11), 'B', Items.gold_ingot}));
            }
            ...
            if (!ExtraUtils.disableTransferNodeEnergyRecipe) {
                addRecipe(new ItemStack(ExtraUtils.transferNode, 1, 12), "GNG", "NRN", "GNG", 'N', new ItemStack(ExtraUtils.transferNode, 1, 0), 'G', Items.gold_ingot, 'R', (ExtraUtils.nodeUpgrade == null) ? Blocks.redstone_block : new ItemStack(ExtraUtils.nodeUpgrade, 1, 8));
                if (ExtraUtils.bedrockiumEnabled) {
        ...
        if (ExtraUtils.nodeUpgradeEnabled) {
            if (!ExtraUtils.disableFilterRecipe) {
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 1, 1), "RSR", "STS", "RSR", 'R', Items.redstone, 'S', Items.stick, 'T', Items.string);
                addRecipe(new RecipeFilterInvert(Item.getItemFromBlock(Blocks.redstone_torch), "Inverted", new ItemStack(ExtraUtils.nodeUpgrade, 1, 1)));
                addRecipe(new RecipeFilterInvert(Item.getItemFromBlock(Blocks.wool), "FuzzyNBT", new ItemStack(ExtraUtils.nodeUpgrade, 1, 1)));
                addRecipe(new RecipeFilterInvert(Items.stick, "FuzzyMeta", new ItemStack(ExtraUtils.nodeUpgrade, 1, 1)));
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 1, 10), "L L", " T ", "L L", 'L', "gemLapis", 'T', new ItemStack(ExtraUtils.nodeUpgrade, 1, 1));
                addRecipe(new RecipeFilterInvert(Item.getItemFromBlock(Blocks.redstone_torch), "Inverted", new ItemStack(ExtraUtils.nodeUpgrade, 1, 10)));
            }
            if (!ExtraUtils.disableNodeUpgradeSpeedRecipe) {
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 4, 0), "RgR", "gGg", "RGR", 'R', Blocks.redstone_block, 'G', Items.gold_ingot, 'g', Items.gold_nugget);
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 1, 2), "LiL", "iPi", "LiL", 'L', new ItemStack(Items.dye, 1, 4), 'i', Items.iron_ingot, 'P', Items.iron_pickaxe);
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 1, 3), "GgG", "DuD", "GDG", 'u', new ItemStack(ExtraUtils.nodeUpgrade, 1, 0), 'G', Items.gold_ingot, 'D', Items.diamond, 'g', Items.gold_nugget);
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 1, 5), "ere", "qeq", "ere", 'e', Items.ender_pearl, 'r', Blocks.redstone_torch, 'q', Items.quartz);
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 1, 6), "ere", "qeq", "eqe", 'e', Items.ender_pearl, 'r', Items.redstone, 'q', Items.quartz);
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 3, 7), "sRR", "sGs", "GRR", 'R', Blocks.redstone_block, 'G', Items.gold_ingot, 'g', Items.gold_nugget, 's', new ItemStack(ExtraUtils.nodeUpgrade, 4, 0));
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 3, 8), "RRR", "sts", "RRR", 'R', Blocks.redstone_block, 'G', Items.gold_ingot, 'g', Items.gold_nugget, 's', new ItemStack(ExtraUtils.nodeUpgrade, 4, 0), 't', new ItemStack(ExtraUtils.nodeUpgrade, 4, 7));
                addRecipe(new ItemStack(ExtraUtils.nodeUpgrade, 3, 9), "RgR", "RGg", "RgR", 'R', Blocks.redstone_block, 'G', Items.gold_ingot, 'g', Items.gold_nugget, 's', new ItemStack(ExtraUtils.nodeUpgrade, 4, 0));
            }
        }
        ...
        if (!ExtraUtils.disableSpikeRecipe) {
            ...
            if (ExtraUtils.spikeGoldEnabled) {
                addRecipe(new ItemStack(ExtraUtils.spikeGold.itemSpike, 4), " S ", "SIS", "ICI", 'S', Items.golden_sword, 'C', Blocks.gold_block, 'I', ExtraUtils.decorative1Enabled ? new ItemStack(ExtraUtils.decorative1, 1, 8) : Items.gold_ingot);
            }
            if (ExtraUtils.spikeDiamondEnabled) {
            ...
            if (ExtraUtils.spikeWoodEnabled) {
                addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.spikeWood.itemSpike, 4), new Object[]{" S ", "SIS", "ICI", 'S', Items.wooden_sword, 'C', "logWood", 'I', "plankWood"}));
            }
        }
        if (ExtraUtils.enderThermicPumpEnabled && !ExtraUtils.disableEnderThermicPumpRecipe) {
            if (!ExtraUtils.decorative1Enabled) {
                ...
                addRecipe(new ItemStack(ExtraUtils.enderThermicPump, 1), "ODO", "LEW", "OPO", 'O', new ItemStack(ExtraUtils.decorative1, 1, 1), 'D', Items.diamond, 'E', Items.ender_eye, 'P', Items.iron_pickaxe, 'L', Items.lava_bucket, 'W', Items.water_bucket);
                addRecipe(new ItemStack(ExtraUtils.enderThermicPump, 1), "ODO", "WEL", "OPO", 'O', new ItemStack(ExtraUtils.decorative1, 1, 1), 'D', Items.diamond, 'E', Items.ender_eye, 'P', Items.iron_pickaxe, 'L', Items.lava_bucket, 'W', Items.water_bucket);
            }
        }
        if (ExtraUtils.enderQuarryEnabled && ExtraUtils.decorative1Enabled) {
            addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.enderQuarry), new Object[]{"EsE", "CDC", "pPp", 'E', new ItemStack(ExtraUtils.decorative1, 1, 1), 's', "treeSapling", 'M', new ItemStack(ExtraUtils.decorative1, 1, 8), 'C', new ItemStack(ExtraUtils.decorative1, 1, 11), 'D', new ItemStack(ExtraUtils.decorative1, 1, 12), 'P', Items.diamond_pickaxe, 'p', (ExtraUtils.enderThermicPump == null) ? new ItemStack(ExtraUtils.decorative1, 1, 12) : ExtraUtils.enderThermicPump}));
        ...
        if (ExtraUtils.enderMarkerEnabled && ExtraUtils.decorative1Enabled) {
            addRecipe(new ItemStack(ExtraUtils.enderMarker), "E", "D", "D", 'E', Items.ender_pearl, 'D', new ItemStack(ExtraUtils.decorative1, 1, 1));
        }
        ...
        if (ExtraUtils.magnumTorchEnabled && !ExtraUtils.disableMagnumTorchRecipe) {
            if (ExtraUtils.chandelier != null) {
                addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.magnumTorch), new Object[]{"RCH", "CWC", "CWC", 'C', ExtraUtils.chandelier, 'W', "logWood", 'R', new ItemStack(Items.potionitem, 1, 8225), 'H', new ItemStack(Items.potionitem, 1, 8229)}));
            } else {
                addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.magnumTorch), new Object[]{"RCH", "CWC", "CWC", 'C', Items.diamond, 'W', "logWood", 'R', new ItemStack(Items.potionitem, 1, 8225), 'H', new ItemStack(Items.potionitem, 1, 8229)}));
            }
        }
        ...
        if (ExtraUtils.goldenLassoEnabled && !ExtraUtils.disableGoldenLassoRecipe) {
            ...
            addRecipe(new RecipeHorseTransmutation());
        }
        if (ExtraUtils.divisionSigilEnabled) {
            addRecipe(new RecipeUnsigil());
        }
        if (ExtraUtils.unstableIngotEnabled) {
            if (!ExtraUtils.disableUnstableIngotRecipe && ExtraUtils.divisionSigilEnabled) {
                addRecipe(new RecipeUnEnchanting());
            ...
            if (ExtraUtils.decorative1Enabled) {
                addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.decorative1, 1, 5), "III", "III", "III", 'I', unstableIngot));
            }
        }
        if (ExtraUtils.portalEnabled && ExtraUtils.underdarkDimID != 0) {
            ...
            if (ExtraUtils.cobblestoneCompr != null) {
                b2 = new ItemStack(ExtraUtils.cobblestoneCompr, 1, 4);
                b3 = new ItemStack(ExtraUtils.cobblestoneCompr, 1, 3);
            } else {
                b2 = new ItemStack(Blocks.diamond_block);
                b3 = new ItemStack(Blocks.diamond_block);
            }
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.portal, 1), "cNc", "NCN", "cNc", 'C', b2, 'c', b3, 'N', u));
        }
        if (ExtraUtils.portalEnabled && ExtraUtils.endoftimeDimID != 0) {
            final Object end_stone = (ExtraUtils.decorative1 == null) ? Blocks.end_stone : new ItemStack(ExtraUtils.decorative1, 1, 14);
            final Object obsidian = (ExtraUtils.decorative1 == null) ? Blocks.obsidian : "burntQuartz";
            addRecipe(new ShapedOreRecipe(new ItemStack(ExtraUtils.portal, 1, 2), new Object[]{"PEP", "ECE", "PEP", 'C', Items.clock, 'E', end_stone, 'P', obsidian}));
        }
        ...
        if (ExtraUtils.ethericSwordEnabled && !ExtraUtils.disableEthericSwordRecipe && ExtraUtils.unstableIngotEnabled) {
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.ethericSword, 1), "I", "I", "S", 'I', unstableIngot, 'S', Blocks.obsidian).setStableItem(ExtraUtils.lawSword));
        }
        if (ExtraUtils.erosionShovelEnabled && !ExtraUtils.disableErosionShovelRecipe && ExtraUtils.unstableIngotEnabled) {
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.erosionShovel, 1), "I", "S", "S", 'I', unstableIngot, 'S', Blocks.obsidian).addStableEnchant(Enchantment.efficiency, 10));
        }
        if (ExtraUtils.destructionPickaxeEnabled && !ExtraUtils.disableDestructionPickaxeRecipe && ExtraUtils.unstableIngotEnabled) {
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.destructionPickaxe, 1), "III", " S ", " S ", 'I', unstableIngot, 'S', Blocks.obsidian).addStableEnchant(Enchantment.efficiency, 10));
        }
        if (ExtraUtils.healingAxeEnabled && !ExtraUtils.disableHealingAxeRecipe && ExtraUtils.unstableIngotEnabled) {
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.healingAxe, 1), "II", "IS", " S", 'I', unstableIngot, 'S', Blocks.obsidian).addStableEnchant(Enchantment.efficiency, 10));
        }
        if (ExtraUtils.temporalHoeEnabled && !ExtraUtils.disableTemporalHoeRecipe && ExtraUtils.unstableIngotEnabled) {
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.temporalHoe, 1), "II", " S", " S", 'I', unstableIngot, 'S', Blocks.obsidian));
        }
        if (ExtraUtils.sonarGogglesEnabled && !ExtraUtils.disableSonarGogglesRecipe && ExtraUtils.unstableIngotEnabled) {
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.sonarGoggles, 1), "III", "EIE", 'I', unstableIngot, 'E', Items.ender_eye));
        }
        if (ExtraUtils.greenScreen != null) {
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.greenScreen, 4, 8), "GGG", "GIG", "GGG", 'I', unstableIngot, 'G', Blocks.stonebrick));
            if (ExtraUtils.colorBlockBrick != null) {
                for (int k = 0; k < 16; ++k) {
                    addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.greenScreen, 4, k), "GGG", "GIG", "GGG", 'I', unstableIngot, 'G', new ItemStack(ExtraUtils.colorBlockBrick, 1, k)));
                }
            }
            final String[] dyes2 = {"Black", "Red", "Green", "Brown", "Blue", "Purple", "Cyan", "LightGray", "Gray", "Pink", "Lime", "Yellow", "LightBlue", "Magenta", "Orange", "White"};
            for (int l = 0; l < 16; ++l) {
                addRecipe(new ShapelessOreRecipe(new ItemStack(ExtraUtils.greenScreen, 1, l), new Object[]{new ItemStack(ExtraUtils.greenScreen, 1, 32767), "dye" + dyes2[15 - l]}));
            }
        }
        if (ExtraUtils.etherealBlockEnabled && !ExtraUtils.disableEtherealBlockRecipe && ExtraUtils.unstableIngotEnabled) {
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.etheralBlock, 4, 0), "GGG", "GIG", "GGG", 'I', unstableIngot, 'G', Blocks.glass));
            if (ExtraUtils.decorative2Enabled) {
                addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.etheralBlock, 4, 1), "GGG", "GIG", "GGG", 'I', unstableIngot, 'G', new ItemStack(ExtraUtils.decorative2, 1, 0)));
                addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.etheralBlock, 4, 2), "GGG", "GIG", "GGG", 'I', unstableIngot, 'G', new ItemStack(ExtraUtils.decorative2, 1, 10)));
            }
            for (int k = 0; k < 3; ++k) {
        ...
        if (ExtraUtils.wateringCanEnabled) {
            if (!ExtraUtils.disableWateringCanRecipe) {
                addRecipe(new ItemStack(ExtraUtils.wateringCan, 1, 1), "SB ", "SWS", " S ", 'S', Items.iron_ingot, 'B', new ItemStack(Items.dye, 1, 15), 'W', Items.bowl);
                addRecipe(RecipeDifficultySpecific.addRecipe(new boolean[]{true, false, false, false}, new ItemStack(ExtraUtils.wateringCan, 1, 1), new String[]{"Peaceful Mode only"}, "S  ", "SWS", " S ", 'S', Blocks.stone, 'W', Items.bowl));
            }
            if (!ExtraUtils.disableSuperWateringCanRecipe && ExtraUtils.soulEnabled && ExtraUtils.bedrockiumEnabled) {
                addRecipe(new ItemStack(ExtraUtils.wateringCan, 1, 3), "SB ", "SWS", " S ", 'S', ExtraUtils.bedrockium, 'B', ExtraUtils.soul, 'W', Items.bowl);
            }
        }
        if (ExtraUtils.scannerEnabled) {
        ...
        if (ExtraUtils.goldenBagEnabled && !ExtraUtils.disableGoldenBagRecipe) {
            ...
            addRecipe(new RecipeBagDye());
            if (ExtraUtils.decorative1Enabled) {
                addRecipe(RecipeGBEnchanting.addRecipe(new ItemStack(ExtraUtils.goldenBag, 1), "WGW", 'd', Items.diamond, 'G', new ItemStack(ExtraUtils.goldenBag, 1), 'W', new ItemStack(ExtraUtils.decorative1, 1, 8)));
            }
        }
        if (ExtraUtils.bedrockiumEnabled) {
            addRecipe(new ItemStack(ExtraUtils.bedrockium, 1), "IcI", "cdc", "IcI", 'I', new ItemStack(ExtraUtils.cobblestoneCompr, 1, 2), 'd', Blocks.diamond_block, 'c', new ItemStack(ExtraUtils.cobblestoneCompr, 1, 3));
        }
        if (ExtraUtils.qedEnabled) {
            addRecipe(new ItemStack(ExtraUtils.qed, 1, 0), "RcR", "EdE", "EEE", 'R', Items.ender_eye, 'c', Blocks.crafting_table, 'd', ExtraUtils.decorative1Enabled ? new ItemStack(ExtraUtils.decorative1, 1, 12) : Blocks.diamond_block, 'E', ExtraUtils.decorative1Enabled ? new ItemStack(ExtraUtils.decorative1, 1, 1) : Blocks.obsidian);
            addRecipe(new ItemStack(ExtraUtils.qed, 1, 2), " e ", " E ", "EEE", 'e', Items.ender_eye, 'd', Items.diamond, 'E', ExtraUtils.decorative1Enabled ? new ItemStack(ExtraUtils.decorative1, 1, 1) : Blocks.obsidian);
        }
        if (!ExtraUtils.disableWitherRecipe) {
            final ItemStack t2 = new ItemStack(Items.nether_star, 1);
            addRecipe(RecipeDifficultySpecific.addRecipe(new boolean[]{true, false, false, false}, t2, new String[]{"Peaceful Mode only"}, "WWW", "SSS", "DSB", 'W', new ItemStack(Items.skull, 1, 1), 'S', Blocks.soul_sand, 'D', Items.diamond_sword, 'B', Items.bow));
        }
        if (ExtraUtils.precisionShearsEnabled && !ExtraUtils.disablePrecisionShears && ExtraUtils.unstableIngotEnabled) {
            addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.precisionShears), "AI", "IA", 'I', unstableIngot, 'B', new ItemStack(ExtraUtils.decorative1, 1, 5), 'A', (ExtraUtils.angelBlock != null) ? ExtraUtils.angelBlock : Blocks.obsidian).addStableEnchant(Enchantment.unbreaking, 10));
        }
        if (ExtraUtils.angelRingEnabled && ExtraUtils.unstableIngotEnabled) {
            ...
            for (int i = 0; i < leftWing.length; ++i) {
                if (ExtraUtils.decorative1Enabled) {
                    addRecipe(RecipeUnstableCrafting.addRecipe(new ItemStack(ExtraUtils.angelRing, 1, i), "LGR", "GNG", "IGI", 'I', unstableIngot, 'G', Items.gold_ingot, 'N', Items.nether_star, 'L', leftWing[i], 'R', rightWing[i]));
                }
                this.addShapelessRecipe(new ItemStack(ExtraUtils.angelRing, 1, i), leftWing[i], new ItemStack(ExtraUtils.angelRing, 1, 32767), rightWing[i]);
            }
        }
        if (!ExtraUtils.disableChestRecipe) {
            GameRegistry.addRecipe(new ShapedOreRecipeAlwaysLast(new ItemStack(Blocks.chest, 4), new Object[]{"WWW", "W W", "WWW", 'W', "logWood"}));
        }
