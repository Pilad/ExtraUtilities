...
public class TileEntityGeneratorNether extends TileEntityGeneratorFurnace {
    ...
    public void doRandomDisplayTickR(final Random random) {
        ...
        if (this.shouldSoundPlay()) {
            ...
            this.worldObj.spawnParticle("mobSpell", this.x() + random.nextFloat(), this.y() + 0.9, this.z() + random.nextFloat(), d0, d2, d3);
            for (int i = 0; i < 50; ++i) {
    ...
    public void doSpecial() {
        if (this.coolDown > 0.0) {
            for (final Object ent : this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(this.x(), this.y(), this.z(), this.x() + 1, this.y() + 1, this.z() + 1).expand(10.0, 10.0, 10.0))) {
