...
public abstract class TransferNodePart extends MCMetaTilePart implements INode, IPipeCosmetic, ISidedHollowConnect, TSlottedPart, TFacePart {
    ...
    public boolean activate(final EntityPlayer player, final MovingObjectPosition part, final ItemStack item) {
        ...
        if (XUHelper.isWrench(item)) {
            final int newmetadata = StdPipes.getNextPipeType(this.getWorld(), part.blockX, part.blockY, part.blockZ, this.getNode().pipe_type);
    ...
    public Iterable<Cuboid6> getCollisionBoxes() {
        ...
        ExtraUtils.transferNode.addCollisionBoxesToList(this.getWorld(), this.x(), this.y(), this.z(), AxisAlignedBB.getBoundingBox(this.x(), this.y(), this.z(), this.x() + 1, this.y() + 1, this.z() + 1), t, null);
        for (final AxisAlignedBB aT : t) {
            t2.add(new Cuboid6(aT.minX, aT.minY, aT.minZ, aT.maxX, aT.maxY, aT.maxZ).sub(new Vector3(this.x(), this.y(), this.z())));
        }
        return t2;
    }
    ...
    public void reloadBlockMasks() {
        ...
        for (final ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            ...
            } else if (!this.tile().canAddPart(TransferNodePart.dummyPipes[dir.ordinal()])) {
                this.blockMasks |= this.flagmasks[dir.ordinal()];
            }
        }
    }
    ...
    public final Cuboid6 getBounds() {
        final Box bounds = ((BlockTransferNode) this.getBlock()).getWorldModel(this.getWorld(), this.x(), this.y(), this.z()).boundingBox();
    ...
    public final HashSet<IndexedCuboid6> getSubParts() {
        ...
        for (final Box bounds : ((BlockTransferNode) this.getBlock()).getWorldModel(this.getWorld(), this.x(), this.y(), this.z())) {
