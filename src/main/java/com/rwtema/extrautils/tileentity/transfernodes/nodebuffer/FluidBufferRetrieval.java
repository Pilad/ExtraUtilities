...
public class FluidBufferRetrieval extends FluidBuffer {
    ...
    public boolean transfer(final TileEntity tile, final ForgeDirection side, final IPipe insertingPipe, final int x, final int y, final int z, final ForgeDirection travelDir) {
        if (tile instanceof IFluidHandler) {
            ...
            return ((TileEntityRetrievalNodeLiquid) this.node.getNode()).fill(this.node.getNodeDir(), destTank.drain(side, drainmax, true), true) <= 0 || ((TileEntityRetrievalNodeLiquid) this.node.getNode()).fill(this.node.getNodeDir(), destTank.drain(side, drainmax, false), false) <= 0;
        }
        return true;
    }
}
