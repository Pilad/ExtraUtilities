...
public class TileEntityTransferNodeEnergy extends TileEntityTransferNode implements INodeEnergy, IEnergyHandler {
    ...
    public void writeToNBT(final NBTTagCompound tag) {
        ...
        if (this.powerHandlerIC2 != null) {
            ...
            tag.setTag("buffer", nbttagcompound1);
        }
    }
    ...
    private void sendEnergy() {
        if (this.clientele.size() > 0) {
            ...
            for (final EnergyPosition client : this.clientele) {
                ...
                final ForgeDirection from = ForgeDirection.getOrientation(client.side);
                if (client.extract) {
            ...
            if (this.powerHandler.getEnergyStored() > 0) {
                for (final EnergyPosition aRepeat : repeat) {
                    ...
                    final TileEntity tile = this.worldObj.getTileEntity(x, y, z);
                    final ForgeDirection from = ForgeDirection.getOrientation(aRepeat.side);
    ...
    private void findMachines() {
        ...
        final IPipe pipeBlock = TNHelper.getPipe(this.worldObj, this.xCoord + this.pipe_x, this.yCoord + this.pipe_y, this.zCoord + this.pipe_z);
        if (pipeBlock != null) {
            for (int i = 0; i < 6; ++i) {
                if (pipeBlock.shouldConnectToTile(this.worldObj, x, y, z, ForgeDirection.getOrientation(i)) || (pipeBlock instanceof INode && ((INode) pipeBlock).getNodeDir() == ForgeDirection.getOrientation(i))) {
                ...
                } else if (TNHelper.doesPipeConnect(this.worldObj, x, y, z, ForgeDirection.getOrientation(i)) && !this.searchLocations.contains(new ChunkPos(this.pipe_x + Facing.offsetsXForSide[i], this.pipe_y + Facing.offsetsYForSide[i], this.pipe_z + Facing.offsetsZForSide[i]))) {
