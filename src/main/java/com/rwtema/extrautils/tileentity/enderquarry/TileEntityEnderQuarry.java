...
public class TileEntityEnderQuarry extends TileEntity implements IEnergyHandler {
    ...
    public static void addUpgradeRecipes() {
        final ItemStack base = new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 0);
        final ItemStack burntQuartz = new ItemStack(ExtraUtils.decorative1, 1, 2);
        final ItemStack endersidian = new ItemStack(ExtraUtils.decorative1, 1, 1);
        ExtraUtils.addRecipe(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 0), " E ", "EQE", " E ", 'E', endersidian, 'Q', burntQuartz);
        ExtraUtils.addRecipe(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 1), " T ", "RBR", 'B', base, 'T', ExtraUtils.trashCan, 'R', Blocks.quartz_block);
        ExtraUtils.addRecipe(new RecipeEnchantCrafting(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 2), new Object[]{" P ", "RBR", 'B', base, 'P', DigType.SILK.newStack(Items.golden_pickaxe), 'R', Items.redstone}));
        ExtraUtils.addRecipe(new RecipeEnchantCrafting(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 3), new Object[]{" P ", "RBR", 'B', base, 'P', DigType.FORTUNE.newStack(Items.iron_pickaxe), 'R', Items.redstone}));
        ExtraUtils.addRecipe(new RecipeEnchantCrafting(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 4), new Object[]{" P ", "RBR", 'B', new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 3), 'P', DigType.FORTUNE.newStack(Items.golden_pickaxe), 'R', Items.redstone}));
        ExtraUtils.addRecipe(new RecipeEnchantCrafting(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 5), new Object[]{"P P", "RBR", 'B', new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 4), 'P', DigType.FORTUNE.newStack(Items.diamond_pickaxe), 'R', Items.redstone}));
        if (ExtraUtils.nodeUpgrade != null) {
            ExtraUtils.addRecipe(new RecipeEnchantCrafting(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 6), new Object[]{" R ", "TBT", 'B', base, 'T', new ItemStack(ExtraUtils.nodeUpgrade, 1, 0), 'R', DigType.SPEED.newStack(Items.diamond_pickaxe)}));
            ExtraUtils.addRecipe(new RecipeEnchantCrafting(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 7), new Object[]{" R ", "TBT", 'B', new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 6), 'T', new ItemStack(ExtraUtils.nodeUpgrade, 1, 0), 'R', DigType.SPEED2.newStack(Items.diamond_pickaxe)}));
            ExtraUtils.addRecipe(new RecipeEnchantCrafting(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 8), new Object[]{"R R", "TBT", 'B', new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 7), 'T', new ItemStack(ExtraUtils.nodeUpgrade, 1, 3), 'R', DigType.SPEED3.newStack(Items.diamond_pickaxe)}));
        }
        ExtraUtils.addRecipe(new ItemStack(ExtraUtils.enderQuarryUpgrade, 1, 9), " T ", "RBR", 'B', base, 'T', Items.bucket, 'R', Items.redstone);
    }
    ...
    public boolean mineBlock(final int x, final int y, final int z, final boolean replaceWithDirt) {
        ...
        if (replaceWithDirt && (block.isLeaves(this.worldObj, x, y, z) || block.isFoliage(this.worldObj, x, y, z) || block.isWood(this.worldObj, x, y, z) || block instanceof IPlantable || block instanceof IGrowable)) {
            if (!this.overClock) {
                this.energy.extractEnergy(TileEntityEnderQuarry.baseDrain, false);
            }
            return true;
        ...
        if (replaceWithDirt && (block == Blocks.grass || block == Blocks.dirt)) {
            ...
            if (TileEntityEnderQuarry.rand.nextInt(16) == 0 && this.worldObj.isAirBlock(x, y + 1, z)) {
                if (TileEntityEnderQuarry.rand.nextInt(5) == 0) {
                    ...
                    this.worldObj.setBlock(x, y + 1, z, Blocks.yellow_flower, TileEntityEnderQuarry.rand.nextInt(BlockFlower.field_149858_b.length), 3);
                } else {
                    this.worldObj.setBlock(x, y + 1, z, Blocks.red_flower, TileEntityEnderQuarry.rand.nextInt(BlockFlower.field_149859_a.length), 3);
                }
            }
            return true;
        }
    ...
    public boolean harvestBlock(final Block block, final int x, final int y, final int z, final int meta, final boolean replaceWithDirt, final DigType digType) {
        ...
        try {
            if (BlockBreakingRegistry.isSpecial(block)) {
                ...
                EventHandlerEntityItemStealer.startCapture(true);
                final boolean flag = this.worldObj.setBlock(x, y, z, (replaceWithDirt && isOpaque) ? (seesSky ? Blocks.grass : Blocks.dirt) : Blocks.air, 0, 3);
                this.items.addAll(EventHandlerEntityItemStealer.getCapturedItemStacks());
                if (!flag) {
                    return false;
                }
                ...
                final float p = ForgeEventFactory.fireBlockHarvesting(i, this.worldObj, block, x, y, z, meta, digType.getFortuneModifier(), 1.0f, digType.isSilkTouch(), fakePlayer);
    ...
    public void startFencing(final EntityPlayer player) {
        ...
        (this.owner = player).addChatComponentMessage(new ChatComponentText("Analyzing Fence boundary"));
    ...
    public enum DigType {
        ...
        DigType(final Enchantment ench, final int level) {
            this.ench = ench;
            this.level = level;
        }
