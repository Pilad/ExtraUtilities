...
public class BlockPortal extends Block implements IBlockLocalization {
    ...
    public BlockPortal() {
        ...
        BlockPortal.darkPortalItemStack = new ItemStack(this, 1, 0);
        BlockPortal.lightPortalItemStack = new ItemStack(this, 1, 2);
    }
    ...
    public boolean onBlockActivated(final World world, final int x, final int y, final int z, final EntityPlayer entity, final int par6, final float par7, final float par8, final float par9) {
        return world.isRemote || this.transferPlayer(world, x, y, z, entity);
    }
        ...
        if (entity.ridingEntity == null && entity.riddenByEntity == null && entity instanceof EntityPlayerMP) {
            ...
            if (type == 0) {
                ...
                if (thePlayer.dimension != ExtraUtils.underdarkDimID) {
                    ...
                    thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, ExtraUtils.underdarkDimID, new TeleporterUnderdark(thePlayer.mcServer.worldServerForDimension(ExtraUtils.underdarkDimID)));
                } else {
                ...
                if (thePlayer.dimension != ExtraUtils.endoftimeDimID) {
                    thePlayer.setLocationAndAngles(x + 0.5, thePlayer.posY, z + 0.5, thePlayer.rotationYaw, thePlayer.rotationPitch);
                    thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, ExtraUtils.endoftimeDimID, new TeleporterEndOfTime(thePlayer.mcServer.worldServerForDimension(ExtraUtils.endoftimeDimID)));
                } else {
                    thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new TeleporterEndOfTime(thePlayer.mcServer.worldServerForDimension(0)));
                }
                return true;
            }
        }
