        ...
        for (final Box b1 : boxes) {
            ...
            if (b1.renderAsNormalBlock) {
                ...
                if (!b1.invisibleSide[0]) {
                    renderer.renderFaceYNeg(block, x + b1.offsetx, y + b1.offsety, z + b1.offsetz, this.getTexture(world, x, y, z, renderer, b1, block, 0, metadata));
                }
                tessellator.setColorOpaque_F(r, g, b2);
                if (!b1.invisibleSide[1]) {
                    renderer.renderFaceYPos(block, x + b1.offsetx, y + b1.offsety, z + b1.offsetz, this.getTexture(world, x, y, z, renderer, b1, block, 1, metadata));
                }
                tessellator.setColorOpaque_F(r * 0.8f, g * 0.8f, b2 * 0.8f);
                renderer.flipTexture = true;
                if (!b1.invisibleSide[2]) {
                    renderer.renderFaceZNeg(block, x + b1.offsetx, y + b1.offsety, z + b1.offsetz, this.getTexture(world, x, y, z, renderer, b1, block, 2, metadata));
                }
                renderer.flipTexture = false;
                if (!b1.invisibleSide[3]) {
                    renderer.renderFaceZPos(block, x + b1.offsetx, y + b1.offsety, z + b1.offsetz, this.getTexture(world, x, y, z, renderer, b1, block, 3, metadata));
                }
                tessellator.setColorOpaque_F(r * 0.6f, g * 0.6f, b2 * 0.6f);
                if (!b1.invisibleSide[4]) {
                    renderer.renderFaceXNeg(block, x + b1.offsetx, y + b1.offsety, z + b1.offsetz, this.getTexture(world, x, y, z, renderer, b1, block, 4, metadata));
                }
                renderer.flipTexture = true;
                if (!b1.invisibleSide[5]) {
                    renderer.renderFaceXPos(block, x + b1.offsetx, y + b1.offsety, z + b1.offsetz, this.getTexture(world, x, y, z, renderer, b1, block, 5, metadata));
                }
                renderer.flipTexture = false;
            }
            renderer.uvRotateBottom = 0;
