    ...
    public void renderSide(final Block block, final double x, final double y, final double z, final double ox, final double oy, final double oz, final int ax, final int ay, final int az, final int bx, final int by, final int bz, final IconConnectedTexture icon, final int side, final double rx, final double ry, final double rz) {
        ...
        if (areSame) {
            ...
            for (int j = 0; j < 4; ++j) {
                ...
                t.addVertexWithUV(cx, cy, cz, icon.getInterpolatedU(16.0 - (8.0 + FakeRenderBlocks.u[j] * 8.0)), icon.getInterpolatedV(16.0 - (8.0 + FakeRenderBlocks.v[j] * 8.0)));
            }
            icon.resetType();
            return;
        }
        for (int j = 0; j < 4; ++j) {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                t.addVertexWithUV(cx + FakeRenderBlocks.u[k] * ax * 0.25 + FakeRenderBlocks.v[k] * bx * 0.25, cy + FakeRenderBlocks.u[k] * ay * 0.25 + FakeRenderBlocks.v[k] * by * 0.25, cz + FakeRenderBlocks.u[k] * az * 0.25 + FakeRenderBlocks.v[k] * bz * 0.25, icon.getInterpolatedU(16.0 - (8.0 + FakeRenderBlocks.u[j] * 4.0 + FakeRenderBlocks.u[k] * 4.0)), icon.getInterpolatedV(16.0 - (8.0 + FakeRenderBlocks.v[j] * 4.0 + FakeRenderBlocks.v[k] * 4.0)));
            }
            icon.resetType();
        }
    }
